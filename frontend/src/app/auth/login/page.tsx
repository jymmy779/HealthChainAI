...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      if (tab === 'phone') {
        ...
        // Format phone to E.164 (e.g. +84901234567)
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      setError(err.message || 'Có lỗi khi kết nối Google');
    }
  };
        ...
        <div className="bg-white rounded-3xl shadow-xl shadow-gray-200/50 p-6 lg:p-8 animate-fadeIn">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  <input
                    ...
                    disabled={loading}
                    className="flex-1 px-4 py-3 bg-gray-50 rounded-xl border border-border focus:border-primary focus:ring-2 focus:ring-primary-light outline-none transition-all text-text-primary text-base disabled:opacity-60"
                  />
                </div>
              </div>
            ) : (
              <>
                <div>
                  ...
                  <input
                    ...
                    disabled={loading}
                    className="w-full px-4 py-3 bg-gray-50 rounded-xl border border-border focus:border-primary focus:ring-2 focus:ring-primary-light outline-none transition-all text-text-primary text-base disabled:opacity-60"
                  />
                </div>
                <div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3.5 bg-primary text-white rounded-2xl font-semibold text-base hover:bg-primary-dark transition-all shadow-lg shadow-primary/25 active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {loading ? 'Đang xử lý...' : (tab === 'phone' ? 'Gửi mã OTP' : 'Đăng nhập')}
            </button>
          </form>
          ...
          <div className="space-y-3">
            <button
              ...
              disabled={loading}
              className="w-full flex items-center justify-center gap-3 py-3 bg-white border-2 border-border rounded-2xl font-medium text-text-primary hover:border-primary/30 transition-all text-base disabled:opacity-50"
            >
            ...
            <Link
              href="/auth/passkey"
              className="w-full flex items-center justify-center gap-3 py-3 bg-white border-2 border-border rounded-2xl font-medium text-text-primary hover:border-primary/30 transition-all text-base text-center block"
            >
